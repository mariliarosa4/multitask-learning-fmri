...
class Network(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, k=10, R=200):
        ...
        self.fc2 = torch.nn.Linear(self.dim2, self.dim4)
        self.bn2 = torch.nn.BatchNorm1d(self.dim4)
        # self.fc3 = torch.nn.Linear(self.dim3, nclass)

        self.heads = torch.nn.ModuleList()
        # for _ in range(self.n_tasks):
        self.heads.append(torch.nn.Linear(self.dim4, 2))
        self.heads.append(torch.nn.Linear(self.dim4, 4))
        self.heads.append(torch.nn.Linear(self.dim4, 1))
    ...
    def forward(self, data):
        ...
        outputs = []
        out1 = F.log_softmax(self.heads[0](x), dim=-1)
        out2 = F.log_softmax(self.heads[1](x), dim=-1)
        out3 = self.heads[2](x)
