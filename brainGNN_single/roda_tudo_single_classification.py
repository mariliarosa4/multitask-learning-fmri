...
def val_loss_single(loader,epoch, model, target_vec):
    ...
    for data in loader:
        ...
        loss_c = F.binary_cross_entropy(output, data.y0.unsqueeze(1))
...
def test_loss_single(test_loader, model, target_vec, name):
    ...
    for data in test_loader:
        ...
        y_pred_list.append(np.round(output.detach().numpy()))
...
def train_single(train_loader, epoch, model, optimizer, train_dataset, writer, target_vec):
    ...
    return loss_all / len(train_dataset),y_pred_list, target_list
...
path_scores_test = '/home/dbserver/Desktop/projetos/repositorios_mestrado/1_mestrado_final/experimentos_finais/GNN/BrainGNN_single/scores/test'
from sklearn.metrics import mean_squared_error, accuracy_score

def main():
    name = 'Single_Gender_v1'   
    ...
    if RECREATE_DATA:
        ...
        labels = func.get_subject_score(phenotype_csv, subject_IDs, score='Gender', regression=False) # 2
    ...
    for fold,(train_idx,test_idx) in enumerate(kfold.split(train_dataset)):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=2, factor=0.1, threshold=1e-4,
                                                                    verbose=True)
        ...
        for epoch in range(1, num_epoch):
            for param_group in optimizer.param_groups:
                print("LR", param_group['lr'])
            tr_loss, y_pred_list_tr, target_list_tr= train_single(train_loader,epoch, model,optimizer, train_dataset, writer, target_vec)
            ...
            print(target_list[0][:10], np.round(y_pred_list[0][:10]))

            print('epoch %i: normalized train loss %0.2f val loss %0.2f' %(epoch, tr_loss, val_loss), end="\r")
            
            val_acc = accuracy_score(target_list[0], np.round(y_pred_list[0]))
            train_acc = accuracy_score(target_list_tr[0], np.round(y_pred_list_tr[0]))
